...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    try {
        ...
        if (hikeMap && hikes.length > 0) {
            hikes.forEach(hike => {
                ...
                const hikeTipHtml =
                    `<span class="map-tip__name">🥾 ${hike.name}</span>` +
                    `<span class="map-tip__stats">${hike.summary_label} · ${hike.difficulty_label}</span>`;
                ...
                const marker = L.marker([hike.start_latitude, hike.start_longitude], { icon: buildHikeDivIcon() })
                    ...
                    .bindTooltip(hikeTipHtml, TIP_OPTS)
                    // Tant que le popup (avec boutons) est ouvert, on retire le tooltip
                    // (sans bouton) pour ne pas afficher deux fois la même rando.
                    .on('popupopen', function () { this.closeTooltip(); this.unbindTooltip(); })
                    .on('popupclose', function () { if (!this.getTooltip()) this.bindTooltip(hikeTipHtml, TIP_OPTS); })
                ...
                if (btn) {
                    ...
                    if (card) {
                        card.addEventListener('mouseenter', () => {
                            ...
                            marker.openTooltip(); // aperçu nom + infos de base sur la carte
            ...
            hikes.forEach(hike => {
                ...
                // Aperçu au survol du tracé lui-même (nom + infos de base), en plus du
                // marqueur de départ. Tooltip « sticky » : il suit le curseur le long du
                // tracé. Masqué quand le popup du tracé est ouvert (pas de doublon).
                const traceTipHtml =
                    `<span class="map-tip__name">🥾 ${hike.name}</span>` +
                    `<span class="map-tip__stats">${hike.summary_label} · ${hike.difficulty_label}</span>`;
                const TRACE_TIP_OPTS = { sticky: true, className: 'map-tip', opacity: 1 };
                const polyline = L.polyline(latlngs, { color: hike.color, weight: 3, opacity: 0.8 })
                    ...
                    .bindTooltip(traceTipHtml, TRACE_TIP_OPTS)
                    .on('popupopen', function () { this.closeTooltip(); this.unbindTooltip(); })
                    .on('popupclose', function () { if (!this.getTooltip()) this.bindTooltip(traceTipHtml, TRACE_TIP_OPTS); })
    ...
    function highlightHikeTrace(hikeId, hovered) {
        ...
        entry.polyline.setStyle(hovered
            ? { color: '#7FC4CC', weight: 5, opacity: 1 }
        ...
        hikingMarkers.forEach(({ marker, hike, polyline }) => {
            ...
            if (polyline) {
                ...
                polyline.setStyle(isSelected
                    ? { color: '#256870', weight: 5, opacity: 1 }
    ...
    // Bouton "Voir" d'une randonnée (ou clic marqueur) : amène à la carte, centre
    // sur le départ de la rando et ouvre son popup. Cohérent avec le "Voir" des services.
    ...
    window.selectHiking = function (hikingId) {
        ...
        // Centrer la carte sur la rando choisie et FERMER le popup (concept unifié
        // avec la ville : après le choix, le popup se ferme ; le marqueur vert + la
        // fiche verte indiquent la sélection).
