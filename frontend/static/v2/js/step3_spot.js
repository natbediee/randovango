    ...
    spots.forEach(spot => {
        ...
        if (map && spot.latitude && spot.longitude) {
            ...
            const marker = L.marker([spot.latitude, spot.longitude], { icon: buildSpotDivIcon(), zIndexOffset: 600 })
                ...
                .bindTooltip(spotTipHtml, TIP_OPTS)
                // Popup ouvert ⇒ on masque le tooltip (pas de double affichage), rétabli à la fermeture.
                .on('popupopen', function () { this.closeTooltip(); this.unbindTooltip(); })
                .on('popupclose', function () { if (!this.getTooltip()) this.bindTooltip(spotTipHtml, TIP_OPTS); })
            ...
            marker._tipHtml = spotTipHtml; // conservé pour re-brancher le tooltip après dé-sélection
...
function createSpotCard(spot, plannedDay) {
    ...
    card.addEventListener('mouseenter', () => {
        ...
        markerData.marker.openTooltip(); // aperçu nom + infos de base
    ...
    card.addEventListener('mouseleave', () => {
        const markerData = spotMarkers.find(m => m.spot.id === spot.id);
        if (markerData) markerData.marker.closeTooltip(); // toujours fermer, même si sélectionné
        const isSelected = localStorage.getItem('selectedSpot') == spot.id;
        if (isSelected) return;
...
function updateSpotSelectionUI(spotId) {
    ...
    spotMarkers.forEach(({ marker, spot }) => {
        ...
        // Spot choisi : pas de tooltip de survol (le marqueur vert suffit) ; rétabli sinon.
        if (isSelected) { marker.closeTooltip(); marker.unbindTooltip(); }
        else if (!marker.getTooltip() && marker._tipHtml) marker.bindTooltip(marker._tipHtml, TIP_OPTS);
